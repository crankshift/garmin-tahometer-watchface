...
- **The outline Gear is a ring just outside the digit**, 1.3 px wide at 260 px, cut to the glyph's advance (at most 1 px on `1`, `4` and `7`).
...
- **Found, not fixed: AM/PM overlaps the Date in 12-hour mode.** `Gear.draw` places AM/PM from `dc.getFontHeight(gearFont)`, which is the font's line height (107 px on `fenix7pro`), not the digits' cap height that the prototype uses. The text lands on the Center Slot's head text, on every watch. Confirmed on `fenix7pro` with the clock forced to 3 pm in 12-hour mode. It predates this ticket, and the always-on view inherits it.
