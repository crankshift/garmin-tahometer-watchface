...
- **Weekday language.** English-only uppercase (per the working rules), so the small font's glyph set is exactly the union of letters used by `MON TUE WED THU FRI SAT SUN AM PM`: `A D E F H I M N O P R S T U W` (computed in `tools/gen_bitmap_font.py`, not hand-typed, to avoid missing a letter).
  - **Changed on 2026-09-25: Polish, Ukrainian and Russian weekdays.** On a Russian Venu 3S the weekday drew as two boxes, because the code took the watch's localized name ("Пт") and SmallFont had no Cyrillic. `Readouts.dateHead` now picks from the face's own tables by `DeviceSettings.systemLanguage`: Polish `NDZ PON WT ŚR CZW PT SOB`, Ukrainian `НД ПН ВТ СР ЧТ ПТ СБ`, Russian `ВС ПН ВТ СР ЧТ ПТ СБ`, and English for every other language. Barlow Condensed has the Polish letters but no Cyrillic, so the Cyrillic ones come from Roboto Condensed Bold (SIL OFL, `ofl/robotocondensed/` in `google/fonts`, a variable font whose Bold instance the generator picks). It is scaled so its capitals are as tall as Barlow's, and its stems measure the same width. No weekday mixes the two fonts. The Latin glyphs' metrics didn't change.
